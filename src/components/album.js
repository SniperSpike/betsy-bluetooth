  ...
  function updatePlaying() {
    playlist.forEach((element) => {
      if (songId === element.songId) {
        ...
        //   dispatch(setColorPalette(colorThief.getPalette(img, 5)));
        // });

        // img.crossOrigin = "Anonymous";
        // // img.src = `http://cors-anywhere.herokuapp.com/http://img.youtube.com/vi/${element.songId}/mqdefault.jpg`;
        // img.src = "https://i1.ytimg.com/vi/DE-DYdK6ISY/maxresdefault";
      }
    });
  }
    ...
    <div id="album-player">
      ...
      <div className="album-details">
        <div className="title-items">
          <ThumbDownAltOutlined onClick={deleteVideo} />
          <span className="title" style={firstTitle}>
            {title}
          </span>
          <ThumbUpAltOutlined />
        </div>
      ...
      <div className="controls">
        ...
        <div className="control-buttons">
          <Loop onClick={() => toggleLoop()} style={{ color: loopingState }} />
          <SkipPrevious onClick={() => skipBackward()} />
          <div className="pause" onClick={() => pause()}>
            {paused ? <PlayArrow /> : <Pause />}
          </div>
          <SkipNext onClick={() => skipForward()} />
          <Shuffle
            onClick={() => toggleShuffle()}
            style={{ color: shuffleState }}
          />
        </div>
      </div>
    </div>
